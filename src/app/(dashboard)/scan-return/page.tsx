...
export default function ScanKembaliPage(): React.JSX.Element {
  const t = useTranslations('transaction')
  const [scannedCode, setScannedCode] = useState('')

  const { data: activeTransaction, isLoading, error } = useActiveTransaction(scannedCode)
  ...
  useEffect(() => {
    if (error && scannedCode.length > 0 && prevErrorRef.current !== error.message) {
      toast.error(t('return.errorNotBorrowed'))
      setScannedCode('')
    }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-3">
        <div className="bg-primary/10 flex size-12 items-center justify-center rounded-2xl">
          <RotateCcw className="text-primary size-6" />
        </div>
        <div>
          <h1 className="text-2xl font-bold tracking-tight">{t('return.title')}</h1>
          <p className="text-muted-foreground text-sm">{t('return.description')}</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2 xl:grid-cols-5">
        ...
        <div className="xl:col-span-3">
          ...
          {/* Active Transaction Found → Show Return Form */}
          {activeTransaction && <ReturnForm activeTransaction={activeTransaction} onSuccess={handleReset} />}
          ...
          {!isLoading && !activeTransaction && (
            <Card className="border-dashed">
              <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-lg font-semibold">{t('empty.returnTitle')}</h3>
                <p className="text-muted-foreground mt-1 max-w-sm text-sm">{t('empty.returnDescription')}</p>
              </CardContent>
            </Card>
          )}
        </div>
